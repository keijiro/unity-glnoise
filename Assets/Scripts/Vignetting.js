...
function OnRenderImage(source : RenderTexture, destination : RenderTexture) {	
	...
	var vignetteColor = RenderTexture.GetTemporary(source.width / 4, source.height / 4, 0);

	Graphics.Blit(source, halfRezColor);
	Graphics.Blit(halfRezColor, quarterRezColor);

	separableBlurMaterial.SetVector("offsets", Vector4(blurSpread * oneOverBaseSize / widthOverHeight, blurSpread * oneOverBaseSize, 0.0, 0.0));
	Graphics.Blit(quarterRezColor, vignetteColor, separableBlurMaterial);

	vignetteMaterial.SetFloat("intensity", intensity);
	vignetteMaterial.SetFloat("blur", blur);
	vignetteMaterial.SetTexture("vignetteTex", vignetteColor);
	Graphics.Blit(source, destination, vignetteMaterial);
	
	RenderTexture.ReleaseTemporary(halfRezColor);			
	RenderTexture.ReleaseTemporary(quarterRezColor);	
	RenderTexture.ReleaseTemporary(vignetteColor);	
}
